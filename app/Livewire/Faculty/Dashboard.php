...
class Dashboard extends Component
{
    ...
    /**
     * Build one printable "LOADS" sheet for the logged-in faculty.
     * OWN loads only, per selected academic term.
     */
    private function buildSheets(): void
    {
        ...
        // ===== OWN-LOADS-ONLY FILTER (meeting-level only; NO offering-level) =====
        $meId          = (int)$me->id;
        ...
        if ($hasSMInstrUID) {
            // Most explicit mapping to users.id
            $q->where('section_meetings.instructor_user_id', $meId);
        } elseif ($hasSMFacId && $hasFacUserId) {
            // faculty_id -> faculties.id -> faculties.user_id -> users.id
            $q->leftJoin('faculties as f', 'f.id', '=', 'section_meetings.faculty_id')
              ->where('f.user_id', $meId);
        } elseif ($hasSMFacId) {
            // fallback: assume faculty_id directly references users.id
            $q->where('section_meetings.faculty_id', $meId);
        } else {
            // no viable mapping columns -> nothing to show
            $this->sheets = [];
            return;
        }
        // ===== /OWN-LOADS-ONLY FILTER =====

        $rows = $q->get();

        // Build one “LOADS” sheet
        $sheet = [
        ...
        foreach ($bucket as $offPack) {
            ...
            foreach ($offPack['curr'] as $c) {
                ...
                $sheet['rows'][] = [
                    ...
                    'title'   => $c['title'],
                    'units'   => $c['units'],
                    'section' => $sectionName,        // Section column
                    'st'      => $st,
                    'et'      => $et,
                    'days'    => $days,
                    'room'    => $room,
