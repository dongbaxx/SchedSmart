...
<div class="print-root p-6 space-y-6">
  ...
  @if(!app('request')->isMethod('post') && ($showHistory ?? false))
    <div class="history-banner print:hidden">
      <div class="flex items-start gap-3">
        ...
        <div class="text-sm">
          <div class="font-semibold">History View (Registrar)</div>
          <div>
  ...
  <form method="GET" action="{{ url()->current() }}" class="flex flex-wrap items-end gap-3 print:hidden">
      ...
      <div>
          ...
          <select name="department" class="border rounded px-3 py-2 w-64">
              <option value="">All Departments</option>
              @foreach($departmentOptions as $opt)
                  <option value="{{ $opt['value'] }}" @selected((int)($departmentId ?? 0) === (int)$opt['value'])>
                      {{ $opt['label'] }}
                  </option>
              @endforeach
          </select>
              ...
              <div class="meta-info">
                    ...
                    <div class="meta-row">
                        <div class="left">
                            Program Name :
                            <strong>
                                {{ !empty($programCourseId) ? ($sheet['course'] ?? '—') : 'ALL' }}
                            </strong>
                        </div>
