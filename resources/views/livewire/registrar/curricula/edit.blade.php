<div class="p-6 space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-semibold">Edit Curriculum</h1>
            <p class="text-sm text-gray-500">Update curriculum details</p>
        </div>

        <a href="{{ route('registrar.curricula.index') }}"
           wire:navigate
    ...
    <form wire:submit.prevent="save" class="bg-white rounded-2xl border p-6 space-y-6">
        <div class="grid md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select wire:model.defer="course_id" class="w-full border rounded-xl px-3 py-2">
                    <option value="">Select course…</option>
                    @foreach($courses as $c)
                        <option value="{{ (string) $c->id }}">{{ $c->name }}</option>
                    @endforeach
                </select>
                @error('course_id') <div class="text-red-600 text-sm">{{ $message }}</div> @enderror
            </div>
